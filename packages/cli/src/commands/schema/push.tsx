import {yellow, blue, red} from 'ansis/colors';
...
export default Command({
    ...
    run: async ({ ctx }) => {
        ...
        logSchemaChangeViolations(data.successful, data.issues, {
            warn: (message, ...args) => console.log(yellow(message), ...args),
            info: (message, ...args) => console.log(blue(message), ...args),
            error: (message, ...args) => console.log(red(message), ...args),
            debug: () => { },
            scope: () => this,
        });
    },
