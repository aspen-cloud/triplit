...
describe('OR queries', () => {
  ...
  it('applies various filter types properly', async () => {
    ...
    {
      const query = db.query('shows').Where(clauseTT);
      const result = await db.fetch(query);
      expect(result.map((r) => r.id)).toContain('1');
    }

    {
      const query = db.query('shows').Where(clauseTF);
      const result = await db.fetch(query);
      expect(result.map((r) => r.id)).toContain('1');
    }

    {
      const query = db.query('shows').Where(clauseFT);
      const result = await db.fetch(query);
      expect(result.map((r) => r.id)).toContain('1');
    }

    {
      const query = db.query('shows').Where(clauseFF);
      const result = await db.fetch(query);
