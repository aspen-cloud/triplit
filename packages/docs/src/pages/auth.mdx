...
Rules are defined as [filter clauses](/fetching-data/queries#filtering) that are applied to a query's result. For example, a write rule `[['author', '=', '$session.SESSION_USER_ID']]` on a collection `todos` would only allow users to assign themselves as the author of a todo.
