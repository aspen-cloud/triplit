...
Many apps require a way to authenticate users. Triplit is configured to look for certain [variables](/query/variables) to determine who is making a request. This information is then used to inform the [authorization](#authorization) process.
...
Rules are defined as [filter clauses](/query/where) that are applied to a query's result. For example, a write rule `[['author', '=', '$session.SESSION_USER_ID']]` on a collection `todos` would only allow users to assign themselves as the author of a todo.
