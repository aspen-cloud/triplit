...
async function updateTriplitWithAuthSession(session) {
  const newToken = session?.access_token;
  // if the token has changed, update the client
  if (newToken !== triplit.token) {
    triplit.disconnect();
    triplit.updateToken(newToken);
    if (!triplit.token) {
...
function getSessionAndSubscribe() {
  supabase.auth.getSession().then(async ({ data: { session } }) => {
    await updateTriplitWithAuthSession(session);
  });
