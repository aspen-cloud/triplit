...
The `TriplitClient`'s session API provides methods to switch out the token and update the client's connection to the server. For best practices on using these APIs and updating user sessions, see the [Auth guide](/auth#changing-users).
