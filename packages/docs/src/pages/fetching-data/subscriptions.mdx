...
If the sync engine is connected to the remote database, subscribing to a query will keep your local database in sync with the remote database as well (depending on the [options](/fetching-data/fetch#fetch-options) provided to the subscription).
