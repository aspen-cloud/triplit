...
Data can be fetched with a single request. The Triplit Client provides two methods `fetch` and `fetchById` which will make a single request to your database. How that request interacts with the local database and remote database can be configured by setting proper [options](#fetch-options).
...
`fetchById()` queries for a single entity by its id and returns the entity if it is found or `undefined` if it is not. For example:
...
const bender = await client.fetchById('employees', 'Bender', options); // undefined
```

`fetchById` is equivalent to building a query with the [entityId](/queries#entity-id) parameter and pulling out the proper result from the result set.
