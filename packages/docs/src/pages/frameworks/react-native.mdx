...
Triplit has a few packages that use features unsupported by the Metro bundler like [exports](https://nodejs.org/docs/latest-v18.x/api/packages.html#package-entry-points). So, Triplit provides a utility for generating a custom Metro config that will resolve these exports.
...
You will need to make sure to install `@triplit/utilities` as a dev dependency (as instructed in step 1):
