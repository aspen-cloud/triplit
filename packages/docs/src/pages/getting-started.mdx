...
This will start a local development server that you can use to test syncing. The `-i` flag will ensure the development server loads the schema defined at `/triplit/schema.ts`.

The default URL for the local development server is `http://localhost:6543`. It will also generate a Service Token and an Anonymous Token that your client app will use to authorize with the server.
...
If you change your schema while the development server is running, you can either restart the server or run `triplit schema push` to update it "live".
...
If you're using a schema **your client will not sync with the server until you apply the schema to it**. You can do this with the Triplit CLI:
