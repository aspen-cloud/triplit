...
A Triplit Client instance will set up a local database for your data and ([if turned on](/syncing-data)) will set up the required network connections for syncing. The specifics of your local database and syncing can be configured by passing in options to the client, which are described in the [client options reference](/syncing-data/client-options). For now we will not pass in any options, which will just set up an in-memory local database.
...
Read about all the data types and options available in a schema [here](/database/schemas).

## Syncing

So far we've covered how to set up a local database and query it. Triplit provides a powerful sync engine that can be used to sync your data across devices in real-time. You can read more about syncing [here](/syncing-data).
...
For more information on safely updating your schema in a production database, see the [schema management guide](/schema-management).
...
- [Remote API, if you prefer HTTP to Websockets](/syncing-data/remote-api)
