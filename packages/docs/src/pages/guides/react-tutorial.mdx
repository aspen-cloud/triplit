...
<Callout emoji="💡">
  ...
  and url shown on the dashbaord for your project page. The instructions in the
  rest of this tutorial are the same whether you're using Triplit Cloud or
  running the sync server locally.
</Callout>

Now's a good time to set up a `.env` file in the `todos` directory. This file will contain the API keys that your app will use to connect to the sync server. Create a new file called `.env` in the `todos` directory and add the following contents:
...
Vite is a build tool that makes building app fast and easy. We already installed Vite above, so let's start using it.
...
Now that we have the development server running, let's integrate Triplit into our client code so that we can read and write data.
...
The Vite app template comes with a `App.tsx` file that contains a some components. Let's replace it with a component that creates a new todo.
...
This component renders a form with a text input and a submit button. When the form is submitted, it calls `triplit.insert` to create a new todo.
...
Create a `components` directory in the `src` directory and a file called `Todo.tsx` and add the following code:
  ...
  return (
    <div>
      <input
        ...
        onChange={() =>
          tripdivt.update('todos', todo.id, async (entity) => {
...
In this component, we're rendering a checkbox and the todo text. The `Todo` component takes a single prop, a `Todo` entity, and renders it. To get the `Todo` type from our schema we use the `Entity` generic type, and pass in our schema and the name of the collection (`'todos'`) that we want a type for.
...
```tsx filename="src/App.tsx" copy showLineNumbers {4, 14, 32-38}
import React, { useState } from 'react';
import { useQuery } from '@triplit/react';
import { triplit } from '../triplit/client';
