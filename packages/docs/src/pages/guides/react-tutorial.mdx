...
This will start the sync server on port `6543` and a database console on port `6542`. It will also output API keys that your app will use to connect to the sync server.

<Callout emoji="💡">
  If you're using our hosted service, Triplit Cloud, you can use the API keys
...
Now's a good time to set up an `.env` file in the `todos` directory. This file will contain the API keys that your app will use to connect to the sync server. Create a new file called `.env` in the `todos` directory and add the following:
...
### Migrate the sync server

Now that we've defined the schema and started the development server, we need to "push" the schema to the sync server. This will allow the sync server to validate the data that we send to it.

First, make sure that the development server is running. Then, in a seperate terminal, in the `todos` directory, run the following commands to migrate the sync server:

```bash copy
npx triplit migrate create initial
npx triplit migrate up
```
...
```bash copy
npx triplit migrate create initial
> Migration file created at ~/todos/triplit/migrations/1705354911962_initial.json
npx triplit migrate up
> Migrating up the sync server:  http://localhost:6543
> applying up migration with id 1705354911962
```

Here's what we just did:

1. We created a new migration file called `1705354911962_initial.json` in the `triplit/migrations` directory. This file translates our initial `schema.ts` to a series of steps that the sync server will use to migrate its database.
2. We applied the migration to the sync server. This will update the sync server's database to match the schema in `schema.ts`. The server will now expect all future clients that connect to it to have the same schema.
...
Triplit provides a client library that you can use to read and write data. Let's initialize it with our API keys and the schema that we defined earlier. Create a new file in the `src` directory called `client.ts` and add the following code:
...
<Callout emoji="💡">
  ...
  can apply filters, sorts, navigate relations and more. You can even edit your
  schema, either by adding new columns or updating/deleting existing ones. Be
  careful - if you make any changes to your schema, they'll go into effect _just
  on the server_ and you'll need to `npx triplit migrate pull` to get your
  client's schema in sync. Read more about migrations
  [here](/guides/migrations).
</Callout>
...
So far, Triplit has been storing data in-memory (the default for `TriplitClient`). That means that if you refresh the page and go offline, you'll lose your data. Triplit supports a variety of [storage options](/client-database/storage) that you can use to persist your data even between refreshes or if you go offline.
...
- The rest of Triplit's [query API](/fetching-data/queries) to select, filter and paginate data
- Triplit's [access control rules](/access-control) to control who can read and write data
- Triplit's [transaction API](/updating-data#transactions)
- Triplit's [relational API](/schemas#relationships) to establish relationships between collections and then [select data across those relationships](/fetching-data/queries#selecting-related-entities)
- The various [storage options](/client-database/storage) for your Triplit client's cache
