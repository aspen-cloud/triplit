...
```typescript
await client.update('employee', 1, async (entity) => {
...
Registers are treated as values and can be assigned to in the updater function.
...
Presently deletes should be handled by users by setting a deleted flag on the entity. The Triplit team is presently evaluating support for deletion.
