...
As a convenience, Triplit provides a method [fetchById](/fetching#fetch) to fetch an entity by its id. It is recommended to use `fetchById` instead of `entityId`.
