...
Triplit uses a [schema](/database/schemas) to define the structure of your data. By using a schema, Triplit can validate your data at runtime and provide autocompletion in your editor.

We're going to set up a [schema](/database/schemas) for the Todos app in the `./triplit/schema.ts` file. Open the file and replace its contents with the following code:
...
So far, Triplit has been storing data in-memory (the default for `TriplitClient`). That means that if you refresh the page and go offline, you'll lose your data. Triplit supports a variety of [storage options](/database/storage) that you can use to persist your data even between refreshes or if you go offline.
...
- Triplit's [relational API](/database/schemas#relationships) to establish relationships between collections and then [select data across those relationships](/fetching-data/queries#selecting-related-entities)
- The various [storage options](/database/storage) for your Triplit client's cache
