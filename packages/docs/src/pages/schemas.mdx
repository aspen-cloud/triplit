...
You may mark attributes in a record as optional with the `Optional` modifier. They will not be required by the schema and will be undefined at runtime if not provided. Optional attributes may also be deleted and assigned to `undefined` in updater functions.

```typescript
import { Schema as S } from '@triplit/db';
const recordTypeWithOptional = S.Record({
  street: S.String(),
  city: S.String(),
  state: S.String(),
  zip: S.String(),
