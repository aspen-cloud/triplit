...
You can provide an array of strings to the `enums` option to restrict the possible values of a string attribute.
...
const schema = {
  test: {
    schema: S.Schema({
      id: S.Id(),
      status: S.String({ enums: ['active', 'inactive'] as const }),
    }),
  },
};
```
