...
An absolute path on the server's file system to a directory where the server will store its database. This is required for durable storage options: `file`, `leveldb`, `lmdb`, and `sqlite`.
